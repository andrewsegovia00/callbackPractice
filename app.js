...
function callbackWordLength(word1, word2) {
    return word1.length - word2.length

}

// The sort method sorts "in place", that is, it modifies the array
words.sort(callbackWordLength);
...
function callbackLongWords(){

}

const longWords = words.filter( e => {e.length >= 7;});
...
function forEach(arr, callbackFunct) {
  for (let i = 0; i < arr.length; i++) {
    callbackFunct(arr[i], i);
  }
}
